...
## 里程碑 1：最小可运行（已完成）
- [x] 时间/模拟核心：tick、time_scale；单步推进 `/simulate/step`；start/stop 循环。
...
- [x] 前端雏形：时间控制（start/stop/step/倍速）、事件流列表、角色列表 + 详情弹窗（简版）。
...
- [ ] 人格漂移：事件/对话后按规则微调性格/关系。
  - [x] 基础：根据事件类型对参与者关系做正/负小幅漂移。
- [ ] 记忆与摘要：角色记忆列表 + 摘要/衰减，限制 prompt 预算。
  - [x] 基础：事件对话自动生成记忆（截断摘要）、角色按上限保留记忆；记忆查询接口。
  - [ ] 进一步：周期性/手动合并摘要后在 prompt 中优先使用 summary（减少上下文）；总结频率/策略配置化。
- [ ] 地图/坐标：map.json（平面 XY）、地点/区域、距离筛选、移动 API 与事件/场景选取。
  - [x] 基础：map.json 读写、地点 coords/描述、角色移动、距离查询；场景/时间线按地点优先挑最近角色。
  - [ ] 视觉与前端：地图展示/编辑；区域（zones）写入与使用；事件/场景距离阈值策略化。
- [ ] 事件线/场景：连贯剧情自动生成与续写。
  - [x] 基础：自动生成场景/时间线，轮询对话，story.log/story.json 记录；无参与者参数时自动挑选角色。
  - [ ] 事件线与外部事件联动：根据近期事件种子生成/分叉事件线；场景结束后的分支/总结；对话重复去重再优化。
...
- [x] LLM 集成：替换 DummyLLM 为真实调用，增加模型/温度/系统提示配置，LLM 超时/stop 及兜底对话。
- [x] 模拟循环控制：支持 start/stop（loop/pause）API，前端可切换持续运行或单步。
- [x] 世界/角色持久化：世界/角色设定写入 `data/worlds/<world_id>/world.json`，启动加载。
- [x] 多世界管理：支持创建新世界、列出/切换世界，按世界目录读写日志与状态。
