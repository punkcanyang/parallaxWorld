...
## 里程碑 1：最小可运行
- [ ] 时间/模拟核心：tick、time_scale；单步推进 `/simulate/step`；暂停/恢复。
- [ ] 世界状态：固定背景 + 1 场景（location）；基础环境字段。
- [ ] 角色：创建角色 API（基本属性/性格/角色标记），列表和查询。
- [ ] 命运事件：简单规则 1-2 个（如随机相遇/早晨问候）；事件队列与调度。
- [ ] 交互/LLM：单一对话模板，能生成一段对话或内心反应。
- [ ] 日志：NDJSON 追加日志（tick、事件、对话），`/logs/tail` 简单过滤。
- [ ] 前端雏形：时间控制（play/pause/倍速）、事件流列表、角色列表 + 详情弹窗（简版）。
- [ ] 技术栈落地（最小版）：后端 Python + FastAPI（复用现有 Parallax/uvicorn 框架），前端 React/Vite；日志先本地文件；LLM 走 Parallax 已有模型（先选轻量模型）。
