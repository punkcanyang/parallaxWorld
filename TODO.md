...
## 里程碑 1：最小可运行
- [x] 时间/模拟核心：tick、time_scale；单步推进 `/simulate/step`；暂停/恢复（loop 已接 start/stop）。
...
- [ ] 世界/角色持久化：将世界与角色设定写入磁盘（如 `data/worlds/<world_id>/world.json`、`characters.json`），启动加载。
- [ ] 多世界管理：支持创建新世界（独立目录）、列出/切换世界、按世界ID加载对应文件。
